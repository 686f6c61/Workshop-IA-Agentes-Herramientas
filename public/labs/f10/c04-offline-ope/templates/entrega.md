# Entrega del kit: Kit F10 C04: offline RL y evaluación contrafactual

## 0. Archivos del ZIP que he usado

- Entradas: `data/logged_policy_events.jsonl` y `data/logged_policy_events_bad.jsonl`.
- Contratos o políticas: `contracts/ope_contract.json`.
- Código ejecutado: `ops/evaluate_offline_policy.py`.
- Evidencias generadas: `output/ope_decision.md`, `output/ope_quality_card.md`, `output/ope_report.json`, `output/estimator_scorecard.csv`, `output/importance_weights.csv` y `output/slice_diagnostics.csv`.

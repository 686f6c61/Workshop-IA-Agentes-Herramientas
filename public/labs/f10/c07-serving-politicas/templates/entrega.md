# Entrega del kit: Kit F10 C07: serving de políticas, drift y rollback

## 0. Archivos del ZIP que he usado

- Entradas: `data/current_window_bad.json`, `data/current_window_ok.json`, `data/reference_window.json` y `data/release_plan.json`.
- Contratos o políticas: `contracts/policy_serving_contract.json`.
- Código ejecutado: `ops/audit_policy_serving.py`.
- Evidencias generadas: `output/serving_decision.md`, `output/serving_runbook.md`, `output/serving_audit_report.json`, `output/drift_scorecard.csv` y `output/rollout_scorecard.csv`.

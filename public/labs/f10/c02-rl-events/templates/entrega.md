# Entrega del kit: Kit F10 C02: eventos RL, trayectorias y replay buffer

## 0. Archivos del ZIP que he usado

- Entradas: `data/events.jsonl` y `data/events_bad.jsonl`.
- Contratos o políticas: `contracts/rl_event_contract.json`.
- Código ejecutado: `ops/validate_rl_events.py`.
- Evidencias generadas: `output/rl_event_decision.md`, `output/rl_event_report.json`, `output/trajectory_snapshot.json` y `output/coverage_state_action.csv`.

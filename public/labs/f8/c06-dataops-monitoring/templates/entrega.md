# Entrega del kit: Kit F8 C06: DataOps, drift y monitorización

## 0. Archivos del ZIP que he usado

- Entradas: `data/production_events.csv`, `data/reference_events.csv` y `data/trace_spans.csv`.
- Contratos o políticas: `contracts/runbook.md`, `contracts/monitoring_contract.json` y `contracts/pipeline_engineering_contract.json`.
- Código ejecutado: `ops/inspect_pipeline_engineering.py` y `ops/monitor_dataops.py`.
- Evidencias generadas: `output/alerts.md`, `output/incident_postmortem.md`, `output/operational_decision.md`, `output/lineage_event.json`, `output/monitoring_report.json` y `output/trace_correlation_report.json`.

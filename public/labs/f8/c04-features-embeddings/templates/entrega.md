# Entrega del kit: Kit F8 C04: features, representaciones y embeddings

## 0. Archivos del ZIP que he usado

- Entradas: `data/feature_cases.csv`, `data/search_queries.csv` y `data/split_assignments.csv`.
- Contratos o políticas: `contracts/feature_contract.json`.
- Código ejecutado: `ops/build_feature_vectors.py` y `ops/search_feature_vectors.py`.
- Evidencias generadas: `output/feature_decision.md`, `output/search_decision.md`, `output/feature_manifest.json`, `output/feature_quality_report.json`, `output/search_report.json` y `output/dense_embedding_matrix.csv`.
